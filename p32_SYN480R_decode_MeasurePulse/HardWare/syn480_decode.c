...
u16 nTIM2_CCR=0;//记录进入TIM2捕获中断的次数
u16 nTIM2_Update=0;//记录进入TIM2更新中断的次数
...
{  
   ...
   //设置该引脚为复用输出功能,输出TIM2 CH3的PWM脉冲波形	GPIOB.5
	...
	GPIO_InitStructure.GPIO_Mode =GPIO_Mode_IPD;  //复用推挽输出GPIO_Mode_IPD
...
static uint16_t HighTime[9];//用于记录脉冲的高电平的时长，单位为10uS
static uint16_t LowTime[9]; //用于记录脉冲的低电平的时长，单位为10uS
...
//void TIM4_IRQHandler(void)
{
	...
	if ( TIM_GetITStatus (TIM2, TIM_IT_CC3 ) != RESET)
	{
		...
		if(Polarity_index==0) //如果现在捕获的是上升沿
		{
			...
			if(tmpHigh>200 && tmpHigh<6000) //收到 同步码头
			{
