...
//void TIM4_IRQHandler(void)
{
	...
	if ( TIM_GetITStatus (TIM2, TIM_IT_CC3 ) != RESET)
	{
		...
		if(Polarity_index==0) //如果现在捕获的是上升沿
		{
			...
			{
				if(syn480DataInfo.nPulse<sizeof(HighTime))
				{
					...
					else//每收到一个字节
					{
						...
						else//收到字节+字节反码
						{
							if(syn480DataInfo.nPulse==16)
							{
								...
								syn480DataInfo.nBytesReceivedPre=syn480DataInfo.byteTmp;
							...
							else
							{
								...
								syn480DataInfo.dataBufferPre[(syn480DataInfo.nPulse>>4)-2]=syn480DataInfo.byteTmp;
							}
...
/**
	...
	*/
	...
	{	
		...
		{
			isON=0;
			syn480DataInfo.ErrNo=2;//数据出错，可能是信号中断
			syn480DataInfo.isPressing=0;//强制停止解码
		}
