...
{ 					
 ...
 //	printf(" LCD ID:%x\r\n",lcddev.id); //打印LCD ID   
	...
	}else if(lcddev.id==0X9331)//OK |/|/|			 
	{
		...
		opt_delay(200); // Dis-charge capacitor power voltage
		LCD_WriteReg(0x0010, 0x1690); // SAP, BT[3:0], AP, DSTB, SLP, STB
		LCD_WriteReg(0x0011, 0x0227); // DC1[2:0], DC0[2:0], VC[2:0]
		opt_delay(50); // Delay 50ms
...
{          
	...
	{
		xlen=ex-sx+1;	 
		for(i=sy;i<=ey;i++)//从上到下，一行一行地填充颜色
		{
		 	LCD_SetCursor(sx,i);      				//设置光标位置 
			LCD_WriteRAM_Prepare();     			//开始写入GRAM	  
			for(j=0;j<xlen;j++)//从左到右填充一行
...
//在指定位置显示一串ASCII字符，必须以字符串结束符结尾,会自动换行
//x,y:起始坐标
//cnString:要显示的ASCII字符串
//size:字体大小，原始大小8*16，size=0或1不放大
//fontColor:字体颜色
//BackColor：背景色
void LCD_ShowAsciiString(u16 x,u16 y,u8* cnString,u8 size,u16 fontColor,u16 BackColor)
{  							  
  u16 temp,i,j;
	u16 y0,px,py;
	u16 TmpIdx;
	...
	while(*cnString != '\0')
	{
		if(x+8*size>lcddev.width)
		{
			x=0;
			y=y+16*size;
		}
		if(y+16*size>lcddev.height)
			return;	//超区域了
	  y0=y;
		TmpIdx=*cnString-' ';
		//asc2_1608[TmpIdx][i]
		...
		for(i=0;i<16;i+=2)
		{ 
			temp=	asc2_1608[TmpIdx][i];
			temp=temp<<8;
			temp+=asc2_1608[TmpIdx][i+1];
			for(j=0;j<16;j++)
			{
				...
				{
					if(temp&0x8000)
						LCD_Fast_DrawPoint(px,py,fontColor);
					else
						LCD_Fast_DrawPoint(px,py,BackColor);
				}		
				temp<<=1;
				y+=size;
				if((y-y0)==16*size)
...
u16 nTime;//测试搜索中文字符的次数

//x,y:起始坐标
//cnString:要显示的汉字字符串
//size:字体大小，原始大小16*16，size=0或1不放大
//fontColor:字体颜色
//BackColor：背景色
void LCD_ShowCnString(u16 x,u16 y,u8* cnString,u8 size,u16 fontColor,u16 BackColor)
{  							  
  u16 temp,i,j;
	u16 y0,px,py;
	u16 TmpIdx=0;//当前结构体数组索引
	u16 TmpIdxPre;//暂存当前索引
	u8 IdxOverFlag;//是否要绕回到前面的数组位置搜索字模
	...
	while(*cnString != '\0')
	{
	  ...
	  y0=y;
		TmpIdxPre=TmpIdx;
		IdxOverFlag=0;
		while((CnChar_1616[TmpIdx].Index[0]!= *cnString)||(CnChar_1616[TmpIdx].Index[1] != *(cnString+1)))
		{
			nTime++;
			TmpIdx++;
			if(TmpIdx==CnCharSize)
			{
				TmpIdx=0;
				IdxOverFlag=1;//绕回到前面的数组位置
			}
			if(IdxOverFlag && TmpIdx==TmpIdxPre)
				return;//找了一圈都找不到字库，就退出这个函数
		}
		
		
		for(i=0;i<16;i++)
		{ 
			temp=	CnChar_1616[TmpIdx].Msk[i];
			temp=temp<<8;
			temp+=CnChar_1616[TmpIdx].Msk[i+16];
			for(j=0;j<16;j++)
			{
				for(px=x;px<x+size;px++)
				for(py=y;py<y+size;py++)
		...
		cnString+=2;
		TmpIdx++;
		TmpIdx=TmpIdx%CnCharSize;//特别注意这里，数组索引不要越界了
	}  	 	  
}   
...
{         
	u8 x0=x;//the start of X position
	width+=x;//the end of X position
	height+=y;
	while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
	{       
