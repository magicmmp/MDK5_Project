...
void SPIx_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOx, ENABLE );//GPIO时钟使能 
	SPI_APBxClock_FUN(RCC_APBxPeriph_SPIx,  ENABLE );//SPI时钟使能 	
 
	GPIO_InitStructure.GPIO_Pin = SPI_SCK_PIN| SPI_MISO_PIN | SPI_MOSI_PIN;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;  //PB13/14/15复用推挽输出 
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(SPI_PORT, &GPIO_InitStructure);//初始化GPIO

 	GPIO_SetBits(SPI_PORT,SPI_SCK_PIN| SPI_MISO_PIN | SPI_MOSI_PIN);  //PB13/14/15上拉
//一般不使用SPI硬件控制CS片选线
